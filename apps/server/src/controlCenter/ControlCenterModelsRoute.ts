...
const GATEWAY_MODELS_PATH = "/api/models";
const GATEWAY_TIMEOUT = "5 seconds";
...
const readModelsRoute = HttpRouter.add(
  "GET",
  CONTROL_CENTER_MODELS_PATH,
  withAuthErrorResponses(
    Effect.gen(function* () {
      ...
      return yield* relayGatewayResponse(httpClient.get(`${gatewayUrl}${GATEWAY_MODELS_PATH}`));
    }),
  ),
);

const writeModelsRoute = HttpRouter.add(
  "POST",
  CONTROL_CENTER_MODELS_PATH,
  withAuthErrorResponses(
    Effect.gen(function* () {
      ...
      return yield* relayGatewayResponse(
        httpClient.post(`${gatewayUrl}${GATEWAY_MODELS_PATH}`, {
...
export const controlCenterModelsRouteLayer = Layer.mergeAll(readModelsRoute, writeModelsRoute);
