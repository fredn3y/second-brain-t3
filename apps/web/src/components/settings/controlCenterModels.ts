...
async function controlCenterRequest(init: RequestInit): Promise<ControlCenterModelsState> {
  const bearerToken = await readDesktopPrimaryBearerToken();
  const headers = new Headers(init.headers);
  if (bearerToken) headers.set("authorization", `Bearer ${bearerToken}`);
  const response = await fetch(resolvePrimaryEnvironmentHttpUrl(CONTROL_CENTER_MODELS_PATH), {
    ...init,
    headers,
  ...
  return record as unknown as ControlCenterModelsState;
}
