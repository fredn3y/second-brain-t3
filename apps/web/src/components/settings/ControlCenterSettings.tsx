    ...
    <SettingsPageContainer>
      ...
      >
        <p className="px-3 pb-1 text-[13px] leading-[1.45] text-muted-foreground/80 sm:px-4">
          Choose which engine, model and effort each Second Brain automation runs with. Every route
          keeps a safe repo default; saving sets a live override and the undo arrow removes it.
        </p>
        ...
        {load.status === "ready"
          ? load.state.consumers.map((route) => (
              <ModelRouteRow
                key={route.consumer}
                route={route}
      ...
      <SettingsSection {...searchableSetting("about-second-brain")}>
        <p className="px-3 text-[13px] leading-[1.45] text-muted-foreground/80 sm:px-4">
          ...
          T3 Code; only the Control Center routing and branding are local.
        </p>
      </SettingsSection>
    </SettingsPageContainer>
  );
